...
class PlayGameState(BaseState):
    ...
    def update(self, game_time, lag):
        ...
        if self.isPlaying and self.player.is_playing():
            increaseY = self.orb_spd * self.fsm.fps_clock.get_time()/1000
        ...
        goalSnapshot = [[] for _ in range(self.laneNo)]
        ...
        for action in actions:
            ...
            if action == "Exit":
                ...
                self.lane_responses[0][0][0] = True
                self.lane_input[0] = True
            elif action == "f (up)":
                self.lane_responses[0][0][0] = False
                self.lane_responses[0][1][0] = False
                self.lane_input[0] = False
            elif action == "g (down)":
                self.lane_responses[1][0][0] = True
                self.lane_input[1] = True
            elif action == "g (up)":
                self.lane_responses[1][0][0] = False
                self.lane_responses[1][1][0] = False
                self.lane_input[1] = False
            elif action == "h (down)":
                self.lane_responses[2][0][0] = True
                self.lane_input[2] = True
            elif action == "h (up)":
                self.lane_responses[2][0][0] = False
                self.lane_responses[2][1][0] = False
                self.lane_input[2] = False
            elif action == "j (down)":
                self.lane_responses[3][0][0] = True
                self.lane_input[3] = True
            elif action == "j (up)":
